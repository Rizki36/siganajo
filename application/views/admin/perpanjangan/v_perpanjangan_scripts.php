<script>
	const $datatable = $('#datatable').DataTable({
		...
		"columnDefs": [],
		"ajax": {
			"url": "<?= base_url('admin/perpanjangan-penahanan/get_datatable'); ?>",
			"type": "POST",
			"data": function(d) {
	...
	function mark_read(id, is_read) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?= base_url('admin/perpanjangan-penahanan/mark_read') ?>",
					data: {
						id,
						is_read
					},
	...
	function detail_tolak(id) {
		$.ajax({
			type: "POST",
			url: "<?= base_url('admin/perpanjangan-penahanan/detail_tolak') ?>",
			data: {
				id
			},
			dataType: "json"
	...
	async function upload(id) {
		...
		if (formValues) {
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url('admin/perpanjangan-penahanan/upload') ?>",
				data: formData,
				dataType: "json",
				processData: false,
				contentType: false,
	...
	function reset(id) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?= base_url('admin/perpanjangan-penahanan/reset') ?>",
					data: {
						id,
					},
					dataType: "json",
