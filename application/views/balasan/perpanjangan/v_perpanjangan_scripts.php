<script>
	const $datatable = $('#datatable').DataTable({
		...
		"columnDefs": [],
		"ajax": {
			"url": "<?= base_url('balasan/perpanjangan-penahanan/get_datatable'); ?>",
			"type": "POST",
			"data": function(d) {
	...
	function detail_tolak(id) {
		$.ajax({
			type: "POST",
			url: "<?= base_url('balasan/perpanjangan-penahanan/detail_tolak') ?>",
			data: {
				id
			},
			dataType: "json"
	...
	function mark_read(id, is_read) {
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?= base_url('balasan/perpanjangan-penahanan/mark_read') ?>",
					data: {
						id,
						is_read
					},
